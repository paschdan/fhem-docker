###############################################################################
# $Id: 71_DENON_AVR_ZONE.pm 10 2017-03-18 00:00:00Z raman $
# 71_DENON_AVR_ZONE
#
...
DENON_AVR_ZONE_Initialize($)
{
	...
	$hash->{AttrList}  = "IODev brand:Denon,Marantz do_not_notify:1,0 disable:0,1 connectionCheck:off,30,45,60,75,90,105,120,240,300 timeout:1,2,3,4,5 ".$readingFnAttributes;
		...
		if($hash->{TYPE} eq "DENON_AVR_ZONE" && $hash->{IODev} eq $IOhash) {
			...
			if ($msg =~ /^(Z2|Z3|Z4)(.+)/) {
				...
				if ($arg eq "ON" || $arg eq "OFF") {
					...
					my $channel = DENON_ZONE_GetKey("CV", $1);
					my $volume = $2;
					if (length($volume) == 2)
					{
						$volume = $volume."0";
					}
					readingsBulkUpdate($zonehash, "level".$channel, ($volume / 10 - 50)." dB");
					$return = "level".$channel . " " .  ($volume / 10 - 50)." dB";
				}
				elsif ($arg =~ /^FAVORITE(.+)/)
				{
				...
				elsif ($msg =~/^PS(.+)/)
				{
					...
					readingsBulkUpdate($zonehash, "volumeStraight", $volume / 10 - 80);
					readingsBulkUpdate($zonehash, "volume", $volume / 10);
...
DENON_AVR_ZONE_Set($$$)
{
	...
	#if(defined($IOhash->{helper}{USEDINPUT}))
	{
		#@inputs = split(/,/,$IOhash->{helper}{USEDINPUT});
	...
	elsif (@$a[1] eq "volumeDown")
	{
		...
		readingsBulkUpdate($hash, "volumeStraight", $volume - 81);
		readingsBulkUpdate($hash, "volume", $volume - 1);
	...
	elsif (@$a[1] eq "volumeUp")
	{
		my $msg = "UP";
		my $volume = ReadingsVal( $name, "volume", "0" );
		readingsBulkUpdate($hash, "volumeStraight", $volume - 79);
		readingsBulkUpdate($hash, "volume", $volume + 1);
...
DENON_AVR_ZONE_Command_Write($$$;$)
{
	...
	readingsBulkUpdate($hash, $call, $state) if(defined($state));
...
DENON_AVR_ZONE_Command_SetVolume($$)
{
	...
	{	
		...
		readingsBulkUpdate($hash, "volumeStraight", $volume - 80);
